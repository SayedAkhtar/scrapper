...
const { logger, scrapperLogger, apiLogger } = require("../logger");
  ...
  do {
    try {
      ...
      body.items.forEach((element) => {
        ...
        if (!lastBatch.includes(element.code)) {
          lastBatch.push(element.code);
          let reelsData = {
            user_id: userId,
            user_name: username,
            reel_id: element.pk,
      ...
      if (reels.length > 0) {
        let status = await postReelsDataToMongo(reels, username);
        if (status) {
...
async function postReelsDataToMongo(req, username = "") {
  ...
  try {
    ...
    apiLogger.debug(`${username} api called: ${++API_CALL_COUNTER}`)
